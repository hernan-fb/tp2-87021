...
pokemon_t *pokemon_crear_desde_string(const char *string) {
  ...
  int parametros_leidos =
      sscanf(string, "%zu,%[^,],%zu,%s", &nuevo_poke->id, nuevo_poke->nombre,
             &nuevo_poke->salud, nuevo_poke->nombre_entrenador);
...
bool pokemon_son_iguales(pokemon_t *pokemon1, pokemon_t *pokemon2) {
  ...
  if (strcmp(pokemon1->nombre_entrenador, pokemon2->nombre_entrenador) != 0) {
    return false;
  }
...
void pokemon_destruir(pokemon_t *pkm) { free(pkm); }
