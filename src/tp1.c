					...
					pokemon_t *poke_a_insertar)
{
    ...
    while (lista_iterador_tiene_siguiente(iter) && ((pokemon_t*)lista_iterador_elemento_actual(iter))->salud < poke_a_insertar->salud) {
...
hospital_t *hospital_crear_desde_archivo(const char *nombre_archivo)
{
	...
	while (fscanf(fp, "%s", buff) != EOF) {
		...
		resultado = pokemon_inserta_ordenado_por_salud(nuevo_hospital->pokemones, poke_aux);
        if (resultado == ERROR) {
			hospital_destruir(nuevo_hospital);
			fclose(fp);
			return NULL;
				 ...
				 size_t cant_pokes_ambulancia)
{
	...
	for (size_t i = 0; i < cant_pokes_ambulancia; i++) {
        pokemon_inserta_ordenado_por_salud(hospital->pokemones, pokemones_ambulancia[i]);
	}
