...
void convertirCapital(char *string) {
    ...
    // Convertir primera letra a mayúscula, independientemente de que la palabra esté subrayada
...
void reemplazarPalabra(char **texto, const char *palabraBuscada, const char *palabraReemplazo) {
    ...
    memmove(posicion + longitudPalabraReemplazo, posicion + longitudPalabraBuscada, strlen(posicion + longitudPalabraBuscada) + 1);
    memcpy(posicion, palabraReemplazo, longitudPalabraReemplazo);
}

void subrayarPalabra(char **texto, const char *palabra_a_Subrayar) {
    char* palabra_subrayada = malloc(sizeof(char)*(strlen(palabra_a_Subrayar)+20));
    if (palabra_subrayada == NULL) {
        return;
    }
    sprintf(palabra_subrayada, "%s%s%s", ANSI_UNDERLINE,palabra_a_Subrayar,ANSI_RESET);
